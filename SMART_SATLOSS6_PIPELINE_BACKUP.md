# SMART SATLOSS6 Pipeline: Complete Embedded Backup

This document is a source-level backup of the canonical DrivAerML SMART
SATLOSS6 pipeline. It embeds the relevant configuration and implementation
modules so a reader does not need to open another repository file to understand
preprocessing, persistent storage, statistics, loading, density estimation,
two-view sampling, SMART inference, supervised losses, consistency, learned
task weighting, AMP/CUDA prefetch, DDP, validation, and checkpointing. The code
blocks below are copied verbatim from the implementation at the time this
backup was created.

## Pipeline Contract

SATLOSS6 uses the same SMART network and seven-plus-four target definition as
vanilla SMART. It differs in training: each batch creates two independent
131072-point geometry views from the same full source geometry using inverse
density sampling. Both views use the same surface and volume query coordinates
and targets. Their supervised Relative-L2 losses are combined with a smooth
worst-view term and detached-midpoint prediction consistency. The canonical
configuration randomizes the two inverse-density beta values independently in
[0, 0.5], uses KDE k=16, disables epoch-seeded vanilla geometry sampling, and
uses learned floor-constrained task weights.
...
raw boundary_<id>.h5 and volume_<id>_filtered.h5
  ...
  -> deterministic train/test or explicit domain split
  -> train-only position and target statistics
  -> memory-mapped full source geometry and query arrays
  -> source-aligned KDE16 log-density cache
  -> independently sampled inverse-density geometry view 1 and view 2
  -> shared surface and volume queries and targets
  -> SMART forward for both views, optionally fused along batch dimension
  -> view supervised losses, mean, smooth worst-view loss
  -> detached-midpoint SmoothL1 consistency with warmup
  -> learned floor-constrained task weighting
  -> AdamW parameter groups, cosine schedule, AMP, CUDA prefetch
  -> aligned, shifted, and robust validation
  -> best and last full-state checkpoints
~~~
...
@hydra.main(version_base="1.2", config_path="config", config_name="drivaerml_satloss6")
...
~~~
### DrivAerML Loader, Statistics, and Density Plumbing
