#!/usr/bin/env python3
"""Compare SMART vs DARM on the same DrivAerML sample and export surface pressure VTK.

This script:
1) Loads one DrivAerML test run.
2) Builds the same encoder geometry input for both SMART and DARM.
3) Builds a 1M-point surface query set (sampling with replacement if needed).
4) Runs both models in 64k surface-query chunks.
5) Exports a pressure-only legacy VTK point cloud with GT, predictions, and abs errors.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compare SMART vs DARM surface pressure on a shared 1M-query export.")
    ...
    p.add_argument("--darm-checkpoint", required=True, help="Path to DARM checkpoint.")
    p.add_argument("--run-id", type=int, default=None, help="Specific DrivAerML test run id. Default: first test run.")
    p.add_argument("--seed", type=int, default=42, help="Sampling seed for shared encoder/query inputs.")
    p.add_argument("--surface-query-count", type=int, default=1_000_000, help="Number of surface queries to evaluate.")
    p.add_argument("--surface-query-chunk", type=int, default=65_536, help="Chunk size used for model surface inference.")
    p.add_argument("--device", default=None, help="Torch device, e.g. cuda:0 or cpu.")
    p.add_argument("--output-dir", default=None, help="Output directory. Default: results/smart_darm_surface_compare/run_<id>")
    return p.parse_args()
...
def write_polydata_vtk(path: Path, points_xyz: np.ndarray, point_data: Dict[str, np.ndarray]) -> None:
    ...
    with open(path, "wb") as f:
        ...
        f.write(b"SMART vs DARM surface pressure comparison\n")
        f.write(b"BINARY\n")
...
def sample_indices(num_items: int, sample_count: int, rng: np.random.Generator, replace: bool | None = None) -> np.ndarray:
    if sample_count <= 0:
...
def run_surface_inference(
    ...
    if hasattr(model, "prediction_query_chunk_size"):
        model.prediction_query_chunk_size = int(chunk_size)

    with torch.inference_mode():
        y_surf_norm, _ = model.inference(
            geo_norm.unsqueeze(0).to(device),
            surf_query_norm.unsqueeze(0).to(device),
            vol_query_norm.unsqueeze(0).to(device),
            None,
        )
        y_surf = denorm_fields(y_surf_norm[0].cpu(), mean_surf, std_surf)
    return y_surf.numpy()
...
def main() -> None:
    ...
    train_data, test_data, stats, spatial_dim, surf_channels, vol_channels, params_dim, fields = get_dataset(smart_cfg)
    if fields["surface"][0] != "pressure":
        raise RuntimeError(f"Expected pressure to be the first surface field, got {fields['surface']}")

    if args.run_id is None:
        run_id = int(test_data.data[0])
    else:
        run_id = int(args.run_id)
    if run_id not in set(int(x) for x in test_data.all_ids):
        raise ValueError(f"run_id={run_id} was not found in the DrivAerML dataset.")
    ...
    if int(smart_cfg.num_body_points) != int(darm_cfg.num_body_points):
        raise ValueError(
            ...
            "Set them equal first if you want truly identical encoder inputs."
        )
    ...
    surf_pressure = np.load(run_dir / "surface_pMeanTrim.npy").astype(np.float32, copy=False).reshape(-1, 1)
    vol_coords = np.load(run_dir / "volume_coords.npy").astype(np.float32, copy=False)
    if vol_coords.shape[0] == 0:
        raise RuntimeError(f"Run {run_id} has no volume points; need one dummy volume query for inference.")

    rng = np.random.default_rng(args.seed + int(run_id))
    shared_geo_idx = sample_indices(
        num_items=surf_coords.shape[0],
        sample_count=int(smart_cfg.num_body_points),
        rng=rng,
        replace=False,
    )
    query_idx = sample_indices(
        num_items=surf_coords.shape[0],
        sample_count=int(args.surface_query_count),
        rng=rng,
        replace=int(args.surface_query_count) > int(surf_coords.shape[0]),
    )

    geo_coords_raw = torch.from_numpy(np.asarray(surf_coords[shared_geo_idx], dtype=np.float32))
    surf_query_coords_raw = torch.from_numpy(np.asarray(surf_coords[query_idx], dtype=np.float32))
    surf_query_pressure_gt = surf_pressure[query_idx, 0].astype(np.float32, copy=False)
    dummy_vol_query_raw = torch.from_numpy(np.asarray(vol_coords[:1], dtype=np.float32))
    ...
    dummy_vol_query_norm = normalize_pos(dummy_vol_query_raw, min_pos, max_pos)
    ...
    smart_pred = run_surface_inference(
        smart_model,
        geo_norm,
        surf_query_norm,
        dummy_vol_query_norm,
        mean_surf,
        std_surf,
        chunk_size=int(args.surface_query_chunk),
        device=device,
    )
    darm_pred = run_surface_inference(
        darm_model,
        geo_norm,
        surf_query_norm,
        dummy_vol_query_norm,
    ...
    metrics = {
        ...
        "surface_query_chunk": int(args.surface_query_chunk),
    ...
    out_root = Path(args.output_dir or (REPO_ROOT / "results" / "smart_darm_surface_compare" / f"run_{run_id}"))
    out_root.mkdir(parents=True, exist_ok=True)

    vtk_path = out_root / "surface_pressure_compare_1M.vtk"
    metrics_path = out_root / "surface_pressure_compare_metrics.json"
    np.save(out_root / "shared_geometry_indices.npy", shared_geo_idx)
    np.save(out_root / "surface_query_indices.npy", query_idx)
    write_polydata_vtk(
        vtk_path,
        surf_query_coords_raw.numpy(),
        {
            "gt_pressure": surf_query_pressure_gt,
            "smart_pred_pressure": smart_pressure,
            "smart_abs_error_pressure": smart_abs_err,
            "darm_pred_pressure": darm_pressure,
            "darm_abs_error_pressure": darm_abs_err,
        },
    )
